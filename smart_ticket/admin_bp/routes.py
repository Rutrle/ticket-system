...
def admin_page():
    ...
    if user_deactivation_form.validate_on_submit() and  current_user.check_attempted_password(user_deactivation_form.password.data):
        user_to_deactivate = User.query.filter_by(id=user_deactivation_form.user_id.data).first()
        if user_to_deactivate.username == user_deactivation_form.user_username.data:
