...
def database_reset() -> None:
    ...
    admin = User(username='admin', email='admin@mail.com', password='admin', user_role=db.session.query(UserRole).filter_by(name="admin").first())
...
def create_dummy_tickets(ticket_num:int = 50)->None:
    ...
    for i in range(ticket_num):
        tickets.append(Ticket(subject=fake.unique.sentence(), issue_description=fake.text(), creation_time=(datetime.now() - timedelta(days=randint(0, 180))), author_id = randint(1,max_user_id)))
...
def create_solved_dummy_tickets(ticket_num:int = 50)->None:
    ...
    for i in range(ticket_num):
        tickets.append(Ticket(subject=fake.unique.sentence(), issue_description=fake.text(), creation_time=(datetime.now() - timedelta(days=randint(180, 360))), author_id = randint(1,max_user_id), solved_on=(datetime.now() - timedelta(days=randint(0, 180))), solver_id=randint(1,max_user_id), is_solved=True))
    ...
    for ticket in tickets:
        ticket_creation_logs.append(TicketLogMessage(ticket_id=ticket.id, message_text="Ticket opened", message_category="sys_message"))
        ticket_creation_logs.append(TicketLogMessage(ticket_id=ticket.id, message_text=fake.text(), message_category="solved"))
...
def create_dummy_users(users_num:int = 50, locale:str = "DE")->None:
    ...
    """
    fake = Faker(locale=locale) # locale is used to prevent phone number extensions to be generated in phone numbers
    users =[]
...
def fill_in_database() -> None:
    ...
    tickets.append(Ticket(subject='Room is too dark',issue_description="Room that I'm sitting in has too many broken lightbulbs so it's dark in it"))
    tickets.append(Ticket(subject='Room is too brigh', issue_description="Room that I'm sitting in has too many lightbulbs so it's too bright in it"))
    tickets.append(Ticket(subject='Too many bugs', issue_description="There are too many bugs in program XYZ"))
    tickets.append(Ticket(subject='Coffee is hot', issue_description="Coffee machine on floor 2 makes coffee too hot", author_id=u1.id))
