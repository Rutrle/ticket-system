...
def database_reset() -> None:
    ...
    then it creates basic user roles
    """
    db.drop_all()
    db.create_all()
...
def fill_in_database() -> None:
    """
    fill the database with dummy data so the functionality of SmartTicket can be seen/tested
    """
    ...
    users.append(User(username='admin', email='admin@mail.com', password='admin',
    ...
    tickets.append(Ticket(subject='Something else broke',nissue_description=long_text))
    tickets.append(Ticket(subject='Lorem ipsum',nissue_description=longer_text))
    tickets.append(Ticket(subject='Owned ticket',nissue_description="I was created by user3", author_id=u.id))
    ...
    tickets.append(Ticket(subject='Room is too dark',nissue_description="Room that I'm sitting in has too many broken lightbulbs so it's dark in it"))
